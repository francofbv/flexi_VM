...
class PretrainVideoMamba(nn.Module):
    ...
    def forward_features(self, x, mask=None, use_image=False):
        ...
        if self.flexible:
            flexible_temporal_pos_embedding = torch.nn.functional.interpolate(
                    self.temporal_pos_embedding.permute(0, 2, 1), size=(T,), mode='linear', align_corners=False
                    )

            # might need this
            flexible_temporal_pos_embedding = flexible_temporal_pos_embedding.permute(0, 2, 1)
